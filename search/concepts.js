(() => {
  ...
  const concepts = [
    ...
    { id: "utility-tree", label: "Utility Tree", category: "framework", aliases: ["utility tree","business value architecture impact","business value architectural impact","high high","h h","utility refinement scenario"], related: ["asr","quality-attribute-scenario","atam"] },
    { id: "asr", label: "ASR", category: "framework", aliases: ["asr","architecture significant requirement","architecturally significant requirement","profound effect on architecture","high business value","architectural impact"], related: ["utility-tree","business-goal","add"] },
    { id: "business-goal", label: "Business Goal", category: "framework", aliases: ["business goal","mission goal","business value","palm","pedigree"], related: ["asr","palm","architecture-influence-cycle"] },
    { id: "palm", label: "PALM", category: "framework", aliases: ["palm","pedigree attribute elicitation method","business goals to quality concerns","pedigree of quality requirements","business goals quality attributes"], related: ["business-goal","asr"] },
    { id: "qaw", label: "QAW", category: "framework", aliases: ["qaw","quality attribute workshop","stakeholders brainstorm scenarios","prioritize scenarios","consolidate scenarios","refine quality scenarios"], related: ["quality-attribute-scenario","asr"] },
    { id: "add", label: "ADD", category: "framework", aliases: ["add","attribute driven design","attribute-driven design","choose element to design","generate and test","asr driven design","decompose element using asrs","quality driven design iteration"], related: ["asr","design-decision"] },
    ...
    { id: "atam", label: "ATAM", category: "framework", aliases: ["atam","architecture tradeoff analysis method","architecture trade-off analysis method","architecture evaluation business drivers","architecture evaluation business drivers utility tree","nine steps","present business drivers","generate utility tree","analyze architectural approaches","brainstorm prioritize scenarios","architecture evaluation method"], related: ["utility-tree","sensitivity-point","trade-off-point","risk-theme","architectural-risk"] },
    { id: "sensitivity-point", label: "Sensitivity Point", category: "framework", aliases: ["sensitivity point","small change large quality effect","decision strongly affects one quality attribute","parameter strongly affects quality"], related: ["atam","trade-off-point"] },
    { id: "trade-off-point", label: "Trade-off Point", category: "framework", aliases: ["trade off point","trade-off point","decision affects multiple quality attributes","competing quality attributes","one decision improves one quality hurts another"], related: ["atam","sensitivity-point"] },
    { id: "architectural-risk", label: "Architectural Risk", category: "risk", aliases: ["architectural risk","atam risk","decision may cause undesirable quality consequence","risk in architecture evaluation"], related: ["atam","risk-theme"] },
    { id: "risk-theme", label: "Risk Theme", category: "risk", aliases: ["risk theme","group of related architectural risks","recurring architectural risk","risk themes"], related: ["atam","architectural-risk"] },
    { id: "architecture-influence-cycle", label: "Architecture Influence Cycle", category: "framework", aliases: ["architecture influence cycle","technical project business professional context","stakeholders organization technical environment","architecture influences its environment","environment influences architecture"], related: ["business-goal","asr"] },
    ...
    { id: "view-documentation", label: "View Documentation", category: "documentation", aliases: ["view documentation","primary presentation","element catalog","context diagram","variability guide","rationale","document a view","view template"], related: ["module-structure","component-and-connector","allocation-structure","cross-view-mapping"] },
    { id: "architecture-notation", label: "Architecture Notation", category: "documentation", aliases: ["architecture notation","informal notation","semiformal notation","semi-formal notation","formal notation","architectural notation","uml style notation","mathematically precise notation"], related: ["view-documentation","cross-view-mapping"] },
    { id: "cross-view-mapping", label: "Cross-View Mapping", category: "documentation", aliases: ["cross view mapping","cross-view mapping","mapping between views","implements is implemented by","one to one mapping","one to many mapping","many to one mapping","map layer view to client server view"], related: ["view-documentation","module-structure","component-and-connector","allocation-structure"] },
    ...
    { id: "publish-subscribe", label: "Publish-Subscribe", category: "architectural-pattern", aliases: ["publish subscribe","publish-subscribe","pubsub","event channel","publisher","subscriber","notification","notifications","distributing notifications","distribute notifications","notification distribution","asynchronous event","event distributor","one event producer informs many","one part sends update many others know","system-level event dissemination"], related: ["observer","component-and-connector","low-coupling"] },
    ...
    { id: "mvc", label: "MVC", category: "architectural-pattern", aliases: ["mvc","model view controller","model-view-controller","model view and controller","ui separation","presentation concerns","controller handles input","multiple views same model","multiple synchronized views","model stores application state","view presents model"] , related: ["observer","publish-subscribe","modifiability"] },
    { id: "map-reduce", label: "Map-Reduce", category: "architectural-pattern", aliases: ["map reduce","map-reduce","map tasks","reduce tasks","distributed processing","partition","parallel computation","large data set","large dataset","shuffle sort","shuffle and sort","key value pairs","batch processing","massive batch data"] , related: ["allocation-structure","pipe-and-filter"] },
    ...
    { id: "observer", label: "Observer", category: "design-pattern", aliases: ["observer","subject observers","many listeners","notify observers","interested objects","state changes","automatically notify many interested objects","listeners updated","subscriber objects","dependent objects automatically updated"] , related: ["publish-subscribe","behavioral"] },
    { id: "state", label: "State", category: "design-pattern", aliases: ["state pattern","state","object changes behavior","internal state","atm state","robot state","avoid conditionals for states","transition","behavior changes with internal state","state specific behavior"] , related: ["behavioral"] },
    { id: "factory-method", label: "Factory Method", category: "design-pattern", aliases: ["factory method","factory","creator","concrete product","object creation","client does not call new","instantiate subclass","creation delegated to subclasses","subclass decides which class to instantiate"] , related: ["creational"] },
    { id: "singleton", label: "Singleton", category: "design-pattern", aliases: ["singleton","single instance","global access point","only one object","private constructor","exactly one instance","one controlled instance"] , related: ["creational"] },
    { id: "composite", label: "Composite", category: "design-pattern", aliases: ["composite","tree structure","part whole","leaf composite","recursive structure","treat individual and group uniformly","treat leaves and composites uniformly"] , related: ["structural"] },
    { id: "facade", label: "Facade", category: "design-pattern", aliases: ["facade","simple interface","hide subsystem complexity","unified interface","shape maker","single simplified interface to complex subsystem"] , related: ["structural","low-coupling"] },
    ...
    { id: "implementation-conformance", label: "Implementation Conformance", category: "testing", aliases: ["implementation conformance","architecture code conformance","architecture-code conformance","implementation violates architecture","implementation code violates documented architecture","implementation code violates architecture","detect architecture violations","detect dependency violations","dependency violations","enforce layering in code","architectural constraints in implementation"] , related: ["architecture-testing","dependency","layer"] },
    { id: "architecture-testing", label: "Architecture-Informed Testing", category: "testing", aliases: ["architecture informed testing","architecture-informed testing","testing guided by architecture","risk based testing","risk-based testing","architecturally significant test paths","unit integration system acceptance regression"] , related: ["implementation-conformance","testability"] },
    { id: "testing-levels", label: "Testing Levels", category: "testing", aliases: ["unit testing","integration testing","system testing","acceptance testing","regression testing","unit integration system acceptance regression"] , related: ["architecture-testing","testability"] },
    { id: "testing-perspectives", label: "Black/White/Gray-Box Testing", category: "testing", aliases: ["black box testing","black-box testing","white box testing","white-box testing","gray box testing","grey box testing","gray-box testing","white-box black-box gray-box"] , related: ["architecture-testing"] },
